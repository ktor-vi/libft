...
int main(void)
{
	...
	/* 2 */ check(ft_strnstr(haystack, needle, SIZE_MAX) == haystack + 1); showLeaks();
	/* 3 */ check(ft_strnstr(haystack, "a", SIZE_MAX) == haystack); showLeaks();
	/* 4 */ check(ft_strnstr(haystack, "c", SIZE_MAX) == haystack + 4); showLeaks();
	/* 5 */ check(ft_strnstr(empty, "", SIZE_MAX) == empty); showLeaks();
	/* 6 */ check(ft_strnstr(empty, "", 0) == empty); showLeaks();
	/* 7 */ check(ft_strnstr(empty, "coucou", SIZE_MAX) == 0); showLeaks();
